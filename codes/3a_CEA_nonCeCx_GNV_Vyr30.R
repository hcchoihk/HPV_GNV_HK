...
folder_proj_noMaleCxFOV_base = "noMaleCx_Vyr30"     ## <- change, e.g., GNV, 2F1M, Vyr30
...
# note. numDose_Vacc=2; originally vacc schedule = 2
numDose_Vacc_new_vec = 2:1
...
# compare 1-dose GNV with shorter protection vs 2-dose FOV with lifelong protection, assume same VE for 1-dose and 2-dose

	print( "diff_calc_1dose_YN==TRUE" )
	print( "compare 1-dose GNV with shorter protection vs 2-dose FOV with lifelong protection, assume same VE for 1-dose and 2-dose" )
	...
	Cost_Vacc_adjfactor_2dose = (Cost_Vacc_use_new/Cost_Vacc_use_base); # no change to numDose, Cost_Vacc per dose may change
	Cost_Vacc_adjfactor_1dose = (1/numDose_Vacc) * (Cost_Vacc_use_new/Cost_Vacc_use_base); # should be same as Cost_Vacc_adjfactor
	...
	Cost_vaccGirls_array_TEMP = Cost_overall_array;
	Cost_vaccBoys_array_TEMP = QALY_overall_array;
	
	
	# variables for Summary; 
	# do the same indexing assignment as if for Cost_overall_array and QALY_overall_array, but on separate variables
	Cost_vaccGirls_array_forSummary = array(0, dim=dim(Cost_array))
	Cost_vaccBoys_array_forSummary = Cost_vaccGirls_array_forSummary;
	Cost_array_CeCx_forSummary = Cost_vaccGirls_array_forSummary;
	QALY_array_CeCx_forSummary = Cost_vaccGirls_array_forSummary;
	
	Costsave_nonCeCx_array_byType_forSummary = rep(list(Cost_array), nonCeCx_num);
	QALYgain_nonCeCx_array_byType_forSummary = Costsave_nonCeCx_array_byType_forSummary;
	LYgain_nonCeCxDeath_array_byType_forSummary = Costsave_nonCeCx_array_byType_forSummary;
	
	# breakdown variables for CeCx
	Cost_CeCx_screening_array_forSummary = array(0, dim=dim(Cost_array))
	Cost_CeCx_treatCancer_array_forSummary = Cost_CeCx_screening_array_forSummary
	Cost_CeCx_treatCancerPalliCare_array_forSummary = Cost_CeCx_screening_array_forSummary
	Cost_CeCx_treatCIN_array_forSummary = Cost_CeCx_screening_array_forSummary
	QALY_CeCx_normal_array_forSummary = Cost_CeCx_screening_array_forSummary
	QALY_CeCx_screening_array_forSummary = Cost_CeCx_screening_array_forSummary
	QALY_CeCx_treatCancer_array_forSummary = Cost_CeCx_screening_array_forSummary
	QALY_CeCx_treatCIN_array_forSummary = Cost_CeCx_screening_array_forSummary	
	
	iVdur_save = 2
	iVC_boys_others = 2:VC_boys_num;
	iVdur_pick_VCboys_Others = 2 # which(Vdur_vec!=100)
	iVC_boys_0 = 1; # VCboys=0 for FOV
	iVdur_pick_VCboys_0 = which(Vdur_vec==100);
	for (iGNV_set in 1:2){
		if (iGNV_set==1){
			# GNV, 1-dose provides a shorter protection
			iVC = iVC_boys_others;
			iVdur_pick = iVdur_pick_VCboys_Others 
		...
		# account nonCeCx
		if (run_SensAnaly_propMaleCx_YN){
			Cost_overall_array[iVC, iVdur_save, ] = Cost_overall_array[iVC, iVdur_save, ] - Costsave_nonCeCx_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]][iVC, iVdur_pick, ] 
			QALY_overall_array[iVC, iVdur_save, ] = QALY_overall_array[iVC, iVdur_save, ] + QALYgain_nonCeCx_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]][iVC, iVdur_pick, ] + LYgain_nonCeCxDeath_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]][iVC, iVdur_pick, ]
		} else{
			Cost_overall_array[iVC, iVdur_save, ] = Cost_overall_array[iVC, iVdur_save, ] - Costsave_nonCeCx_array[iVC, iVdur_pick, ] 
			QALY_overall_array[iVC, iVdur_save, ] = QALY_overall_array[iVC, iVdur_save, ] + QALYgain_nonCeCx_array[iVC, iVdur_pick, ] + LYgain_nonCeCxDeath_array[iVC, iVdur_pick, ]
		}
		...
		Cost_vaccGirls_array_forSummary[iVC, iVdur_save, ] = Cost_vaccGirls_array_TEMP[iVC, iVdur_save, ]; # use iVdur_save because it uses Cost_vaccGirls_array_TEMP
		Cost_vaccBoys_array_forSummary[iVC, iVdur_save, ] = Cost_vaccBoys_array_TEMP[iVC, iVdur_save, ]; # use iVdur_save because it uses Cost_vaccBoys_array_TEMP	
		Cost_array_CeCx_forSummary[iVC, iVdur_save, ] = Cost_array[iVC, iVdur_pick, ];
		QALY_array_CeCx_forSummary[iVC, iVdur_save, ] = QALY_array[iVC, iVdur_pick, ];
		for (inonCeCx in 1:nonCeCx_num){
			if (inonCeCx %in% inonCeCx_adjMSM_vec & run_SensAnaly_propMaleCx_YN){ 
				Costsave_nonCeCx_array_byType_forSummary[[inonCeCx]][iVC, iVdur_save, ] = Costsave_nonCeCx_array_byType_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][iVC, iVdur_pick, ]
				QALYgain_nonCeCx_array_byType_forSummary[[inonCeCx]][iVC, iVdur_save, ] = QALYgain_nonCeCx_array_byType_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][iVC, iVdur_pick, ]
				LYgain_nonCeCxDeath_array_byType_forSummary[[inonCeCx]][iVC, iVdur_save, ] = LYgain_nonCeCxDeath_array_byType_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][iVC, iVdur_pick, ]
			} else{
				Costsave_nonCeCx_array_byType_forSummary[[inonCeCx]][iVC, iVdur_save, ] = Costsave_nonCeCx_array_byType[[inonCeCx]][iVC, iVdur_pick, ]
				QALYgain_nonCeCx_array_byType_forSummary[[inonCeCx]][iVC, iVdur_save, ] = QALYgain_nonCeCx_array_byType[[inonCeCx]][iVC, iVdur_pick, ]
				LYgain_nonCeCxDeath_array_byType_forSummary[[inonCeCx]][iVC, iVdur_save, ] = LYgain_nonCeCxDeath_array_byType[[inonCeCx]][iVC, iVdur_pick, ]
			}
		}
		Cost_CeCx_screening_array_forSummary[iVC, iVdur_save, ] = Cost_CeCx_screening_array[iVC, iVdur_pick, ]
		Cost_CeCx_treatCancer_array_forSummary[iVC, iVdur_save, ] = Cost_CeCx_treatCancer_array[iVC, iVdur_pick, ]
		Cost_CeCx_treatCancerPalliCare_array_forSummary[iVC, iVdur_save, ] = Cost_CeCx_treatCancerPalliCare_array[iVC, iVdur_pick, ]
		Cost_CeCx_treatCIN_array_forSummary[iVC, iVdur_save, ] = Cost_CeCx_treatCIN_array[iVC, iVdur_pick, ]
		QALY_CeCx_normal_array_forSummary[iVC, iVdur_save, ] = QALY_CeCx_normal_array[iVC, iVdur_pick, ]
		QALY_CeCx_screening_array_forSummary[iVC, iVdur_save, ] = QALY_CeCx_screening_array[iVC, iVdur_pick, ]
		QALY_CeCx_treatCancer_array_forSummary[iVC, iVdur_save, ] = QALY_CeCx_treatCancer_array[iVC, iVdur_pick, ]
		QALY_CeCx_treatCIN_array_forSummary[iVC, iVdur_save, ] = QALY_CeCx_treatCIN_array[iVC, iVdur_pick, ]	
	
	} # for-iGNV_set


	# for TVC calculation
	Cost_overall_array_woVacc = Cost_overall_array - Cost_vaccGirls_array_TEMP - Cost_vaccBoys_array_TEMP # overall cost without vaccination cost; old code = Cost_array - Costsave_nonCeCx_array
	...
	# Cost_overall_2doseFOV/1doseFOV are used later
	# for the potential to compare 1-dose FOV vs 2-dose FOV
	iVC = 1 # which(VC_boys_vec==0) and so Cost_vaccBoys_array[iVC, iVdur_iVC0_FOV, ] should be 0
	iVdur_iVC0_FOV = which(Vdur_vec==100) # lifelong protection
	if (run_SensAnaly_propMaleCx_YN){
		# left hand side uses iVdur = iVdur_iVC0_FOV 
		Cost_overall_2doseFOV = Cost_array[iVC, iVdur_iVC0_FOV, ] + Cost_Vacc_adjfactor_2dose*Cost_vaccGirls_array[iVC, iVdur_iVC0_FOV, ] - Costsave_nonCeCx_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]][iVC, iVdur_iVC0_FOV, ] # + Cost_Vacc_adjfactor_2dose*Cost_vaccBoys_array[iVC, iVdur_iVC0_FOV, ]
		QALY_overall_2doseFOV = QALY_array[iVC, iVdur_iVC0_FOV, ] + QALYgain_nonCeCx_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]][iVC, iVdur_iVC0_FOV, ] + LYgain_nonCeCxDeath_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]][iVC, iVdur_iVC0_FOV, ]

		iVdur = 2 # which(Vdur_vec!=100) # non-lifelong protection
		Cost_overall_1doseFOV = Cost_array[iVC, iVdur, ] + Cost_Vacc_adjfactor_1dose*Cost_vaccGirls_array[iVC, iVdur, ] - Costsave_nonCeCx_array[iVC, iVdur, ] # + Cost_Vacc_adjfactor_1dose*Cost_vaccBoys_array[iVC, iVdur, ]
		QALY_overall_1doseFOV = QALY_array[iVC, iVdur, ] + QALYgain_nonCeCx_array[iVC,iVdur, ] + LYgain_nonCeCxDeath_array[iVC, iVdur, ]
	} else{
		# left hand side uses iVdur = iVdur_iVC0_FOV 
		Cost_overall_2doseFOV = Cost_array[iVC, iVdur_iVC0_FOV, ] + Cost_Vacc_adjfactor_2dose*Cost_vaccGirls_array[iVC, iVdur_iVC0_FOV, ] - Costsave_nonCeCx_array[iVC, iVdur_iVC0_FOV, ] # + Cost_Vacc_adjfactor_2dose*Cost_vaccBoys_array[iVC, iVdur_iVC0_FOV, ]
		QALY_overall_2doseFOV = QALY_array[iVC, iVdur_iVC0_FOV, ] + QALYgain_nonCeCx_array[iVC, iVdur_iVC0_FOV, ] + LYgain_nonCeCxDeath_array[iVC, iVdur_iVC0_FOV, ]
	...
	diff_Cost_CeCx_treatCxOnly_vsNoVacc_byVdur[[iVdur]] = pracma::repmat(Cost_CeCx_treatCancer_array[iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1) - Cost_CeCx_treatCancer_array[VC_boys_comp_T, iVdur_pick_VCboys_Others, ]
	diff_QALYgain_CeCx_treatCxOnly_vsNoVacc_byVdur[[iVdur]] = pracma::repmat(QALY_CeCx_treatCancer_array[iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1) - QALY_CeCx_treatCancer_array[VC_boys_comp_T, iVdur_pick_VCboys_Others, ]
	
	# nonCeCx, gwarts, _Costsave_ and _QALYgain_ are recorded, so the benefit of VCboys is VCboys_others - VCboys_0
	for (inonCeCx in 1:nonCeCx_num){
		if (inonCeCx %in% inonCeCx_adjMSM_vec & run_SensAnaly_propMaleCx_YN){
			diff_Costsave_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]] = 		Costsave_MaleCx_array_adjMaleCxFOV_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(Costsave_MaleCx_array_adjMaleCxFOV_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
			diff_QALYgain_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]] = 		QALYgain_MaleCx_array_adjMaleCxFOV_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(QALYgain_MaleCx_array_adjMaleCxFOV_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
			diff_LYgain_nonCeCxDeath_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]] = 		LYgain_MaleCxDeath_array_adjMaleCxFOV_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(LYgain_MaleCxDeath_array_adjMaleCxFOV_bypMaleCx[[inonCeCx]][[iprop_MaleCx]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
		} else {
			diff_Costsave_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]] = 		Costsave_nonCeCx_array_byType[[inonCeCx]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(Costsave_nonCeCx_array_byType[[inonCeCx]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
			diff_QALYgain_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]] = 		QALYgain_nonCeCx_array_byType[[inonCeCx]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(QALYgain_nonCeCx_array_byType[[inonCeCx]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
			diff_LYgain_nonCeCxDeath_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]] = 		LYgain_nonCeCxDeath_array_byType[[inonCeCx]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(LYgain_nonCeCxDeath_array_byType[[inonCeCx]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
		}
	...
	if (calc_gwarts_YN==1){
		for (isex in 1:sex_num){
			diff_Costsave_gwarts_vsNoVacc_bySex_byVdur[[iVdur]][[isex]] = 		Costsave_gwarts_array_bysex[[isex]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(Costsave_gwarts_array_bysex[[isex]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
			diff_QALYgain_gwarts_vsNoVacc_bySex_byVdur[[iVdur]][[isex]] = 		QALYgain_gwarts_array_bysex[[isex]][VC_boys_comp_T, iVdur_pick_VCboys_Others, ] - pracma::repmat(QALYgain_gwarts_array_bysex[[isex]][iVC_boys_0, iVdur_pick_VCboys_0, ], n=VC_boys_num,m=1)
		} # end isex
	} # if-calc_gwarts_YN==1
	
	# set diff=0 to iVC_boys_0
	diff_Cost_CeCx_vsNoVacc_byVdur[[iVdur]][iVC_boys_0, ] = 0;
	diff_QALYgain_CeCx_vsNoVacc_byVdur[[iVdur]][iVC_boys_0, ] = 0;
	diff_Cost_CeCx_treatCxOnly_vsNoVacc_byVdur[[iVdur]][iVC_boys_0, ] = 0;
	diff_QALYgain_CeCx_treatCxOnly_vsNoVacc_byVdur[[iVdur]][iVC_boys_0, ] = 0;
	for (inonCeCx in 1:nonCeCx_num){
		diff_Costsave_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]][iVC_boys_0, ] = 0;
		diff_QALYgain_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]][iVC_boys_0, ] = 0;
		diff_LYgain_nonCeCxDeath_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]][iVC_boys_0, ] = 0;
		diff_QALYgain_plusLYgainDeath_nonCeCx_vsNoVacc_byType_byVdur[[iVdur]][[inonCeCx]][iVC_boys_0, ] = 0;
	} # end inonCeCx
	if (calc_gwarts_YN==1){
		for (isex in 1:sex_num){
			diff_Costsave_gwarts_vsNoVacc_bySex_byVdur[[iVdur]][[isex]][iVC_boys_0, ] = 0;
			diff_QALYgain_gwarts_vsNoVacc_bySex_byVdur[[iVdur]][[isex]][iVC_boys_0, ] = 0;
		} # end isex
	} # if-calc_gwarts_YN==1
	
	# gwarts cases prevented, should equal to 2d-FOV when VC_boys==0
	if (calc_gwarts_YN==1){
		if (Vdur_num==2){
			for (isex in 1:sex_num){
				Caseprev_gwarts_array_bysex[[isex]][1, 2, ] = Caseprev_gwarts_array_bysex[[isex]][1, 1, ];
				relative_Caseprev_gwarts_array_bysex[[isex]][1, 2, ] = relative_Caseprev_gwarts_array_bysex[[isex]][1, 1, ];
				# for ageCaseprev
				for (iage_Caseprev_gw in 1:gwarts_Caseprev_age_vec_num){
					Caseprev_gwarts_array_bysex_age_list[[iage_Caseprev_gw]][[isex]][1, 2, ] = Caseprev_gwarts_array_bysex_age_list[[iage_Caseprev_gw]][[isex]][1, 1, ];
					relative_Caseprev_gwarts_array_bysex_age_list[[iage_Caseprev_gw]][[isex]][1, 2, ] = relative_Caseprev_gwarts_array_bysex_age_list[[iage_Caseprev_gw]][[isex]][1, 1, ];
				} # for- iage_Caseprev_gw
			}
		} # if- Vdur_num==2
	} # if- calc_gwarts_YN==1

# diff_calc_1dose_YN==TRUE & numDose_Vacc_new==1
	...
	if (run_SensAnaly_propMaleCx_YN){
		Cost_overall_array = Cost_array + Cost_Vacc_adjfactor*Cost_vaccGirls_array + Cost_Vacc_adjfactor*Cost_vaccBoys_array - Costsave_nonCeCx_array_adjMaleCxFOV_bypMaleCx[[iprop_MaleCx]]
		...
		Cost_overall_array = Cost_array + Cost_Vacc_adjfactor*Cost_vaccGirls_array + Cost_Vacc_adjfactor*Cost_vaccBoys_array - Costsave_nonCeCx_array
...
#	print( "caseprev_gwarts" ); browser();
		...
		for (isex in 1:sex_num){
			...
			for (iage_Caseprev_gw in 1:gwarts_Caseprev_age_vec_num){
				...
				print( "additional relative gwarts prevented, compared to VCboys=0" )
				if (diff_calc_1dose_YN==TRUE & numDose_Vacc_new==1){
					print( "recall. for comparing 1d-GNV to 2d-FOV, VC_boys==0 refers to 2d-FOV" );
				}
				relative_Caseprev_gwarts_summ_Temp = do.call(rbind, sapply(2:VC_boys_num, simplify=FALSE, function(iiVCboys)
...
for (iVdur in Vdur_vec_comp){ # Vdur_vec_comp would change for 1-dose GNV vs 2-dose FOV
	...
	if (diff_calc_1dose_YN==TRUE & numDose_Vacc_new==1){ # print summary items for diff_calc_1dose_YN
		...
		print( "Cost_vaccGirls; Cost_vaccGirls_array_forSummary" )
			print( t(apply(Cost_vaccGirls_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "Cost_vaccBoys; Cost_vaccBoys_array_forSummary" )
			print( t(apply(Cost_vaccBoys_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )		
		print( "Cost_CeCx only; Cost_array_CeCx_forSummary" )
			print( t(apply(Cost_array_CeCx_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "QALY_CeCx only; QALY_array_CeCx_forSummary" )
			print( t(apply(QALY_array_CeCx_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )

		# Costsave / QALYgain based on the actual cost / QALY
		cat("\n"); print( "Costsave / QALYgain for vaccGirls/Boys and CeCx, compared to _iVCboys=0_." )
		print( "Costsave vaccGirls; from Cost_vaccGirls_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_vaccGirls_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_vaccGirls_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "Costsave vaccBoys; from Cost_vaccBoys_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_vaccBoys_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_vaccBoys_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "Costsave vaccGirls + vaccBoys; from Cost_vaccGirls/vaccBoys_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_vaccGirls_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_vaccGirls_array_forSummary[-1, iVdur, ]) + 		pracma::repmat(Cost_vaccBoys_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_vaccBoys_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "Costsave CeCx only; from Cost_array_CeCx_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_array_CeCx_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_array_CeCx_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "QALYgain CeCx only; from QALY_array_CeCx_forSummary" )
			QALYgain_array_TEMP = as.matrix(QALY_array_CeCx_forSummary[-1, iVdur, ]) - pracma::repmat(QALY_array_CeCx_forSummary[1, iVdur, ], VC_boys_num-1, 1);
			print( t(apply(QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )
		
		
		# breakdown variables for CeCx
		cat("\n"); print( "breakdown variables for CeCx" )
		print( "Cost_CeCx_screening; Cost_CeCx_screening_array_forSummary" )
			print( t(apply(Cost_CeCx_screening_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "Cost_CeCx_treatCancer; Cost_CeCx_treatCancer_array_forSummary" )
			print( t(apply(Cost_CeCx_treatCancer_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "Cost_CeCx_treatCancer; Cost_CeCx_treatCancerPalliCare_array_forSummary" )
			print( t(apply(Cost_CeCx_treatCancerPalliCare_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "Cost_CeCx_treatCIN; Cost_CeCx_treatCIN_array_forSummary" )
			print( t(apply(Cost_CeCx_treatCIN_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "QALY_CeCx_normal; QALY_CeCx_normal_array_forSummary" )
			print( t(apply(QALY_CeCx_normal_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "QALY_CeCx_screening; QALY_CeCx_screening_array_forSummary" )
			print( t(apply(QALY_CeCx_screening_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "QALY_CeCx_treatCancer; QALY_CeCx_treatCancer_array_forSummary" )
			print( t(apply(QALY_CeCx_treatCancer_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )
		print( "QALY_CeCx_treatCIN; QALY_CeCx_treatCIN_array_forSummary" )
			print( t(apply(QALY_CeCx_treatCIN_array_forSummary[, iVdur, ],1, simplify=TRUE, summary_new)) )	
		
		# Costsave / QALYgain based on the actual cost / QALY
		print( "breakdown variables for CeCx, difference, compared to iVC_boys=0" )
		print( "Costsave CeCx_screening; from Cost_CeCx_screening_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_CeCx_screening_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_CeCx_screening_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "Costsave CeCx_treatCancer; Cost_CeCx_treatCancer_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_CeCx_treatCancer_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_CeCx_treatCancer_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "Costsave CeCx_treatCancerPalliCare; Cost_CeCx_treatCancerPalliCare_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_CeCx_treatCancerPalliCare_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_CeCx_treatCancerPalliCare_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "Costsave CeCx_treatCIN; Cost_CeCx_treatCIN_array_forSummary" )
			Costsave_array_TEMP = pracma::repmat(Cost_CeCx_treatCIN_array_forSummary[1, iVdur, ], VC_boys_num-1, 1) - as.matrix(Cost_CeCx_treatCIN_array_forSummary[-1, iVdur, ]);
			print( t(apply(Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "QALYgain CeCx_normal; QALY_CeCx_normal_array_forSummary" )
			QALYgain_array_TEMP = as.matrix(QALY_CeCx_normal_array_forSummary[-1, iVdur, ]) - pracma::repmat(QALY_CeCx_normal_array_forSummary[1, iVdur, ], VC_boys_num-1, 1);
			print( t(apply(QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "QALYgain CeCx_screening; from QALY_CeCx_screening_array_forSummary" )
			QALYgain_array_TEMP = as.matrix(QALY_CeCx_screening_array_forSummary[-1, iVdur, ]) - pracma::repmat(QALY_CeCx_screening_array_forSummary[1, iVdur, ], VC_boys_num-1, 1);
			print( t(apply(QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )
		print( "QALYgain CeCx_treatCancer; from QALY_CeCx_treatCancer_array_forSummary" )
			QALYgain_array_TEMP = as.matrix(QALY_CeCx_treatCancer_array_forSummary[-1, iVdur, ]) - pracma::repmat(QALY_CeCx_treatCancer_array_forSummary[1, iVdur, ], VC_boys_num-1, 1);
			print( t(apply(QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )

		print( "QALYgain CeCx_treatCIN; from QALY_CeCx_treatCIN_array_forSummary" )
			QALYgain_array_TEMP = as.matrix(QALY_CeCx_treatCIN_array_forSummary[-1, iVdur, ]) - pracma::repmat(QALY_CeCx_treatCIN_array_forSummary[1, iVdur, ], VC_boys_num-1, 1);
			print( t(apply(QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )
		
		
		# nonCeCx 
		cat("\n"); print( "nonCeCx items" )
		print( "original Costsave / QALYgain refers to the casediff compared to completely noVacc (VC=0 for girls and boys)." )
		for (inonCeCx in 1:nonCeCx_num){
			print( sprintf("inonCeCx: %d, %s", inonCeCx, nonCeCx_vec[[inonCeCx]]) )
			print( "Costsave_nonCeCx; Costsave_nonCeCx_array_byType_forSummary" )
				print( t(apply(Costsave_nonCeCx_array_byType_forSummary[[inonCeCx]][, iVdur, ],1, simplify=TRUE, summary_new)) )
			print( "QALYgain_nonCeCx; QALYgain_nonCeCx_array_byType_forSummary" )
				print( t(apply(QALYgain_nonCeCx_array_byType_forSummary[[inonCeCx]][, iVdur, ],1, simplify=TRUE, summary_new)) )
			print( "LYgain_nonCeCx; LYgain_nonCeCxDeath_array_byType_forSummary" )
				print( t(apply(LYgain_nonCeCxDeath_array_byType_forSummary[[inonCeCx]][, iVdur, ],1, simplify=TRUE, summary_new)) )
		} # end-inonCeCx
		
		# difference in Costsave / QALYgain based on the actual Costsave / QALYgain that were compared to no vaccination. So, the direction for diff_Costsave whould be Costsave_xVC_boys - Costsave_0VC_boys.
		print( "_difference_ Costsave / QALYgain, compared to _iVC_boys=0_." )
		for (inonCeCx in 1:nonCeCx_num){
			print( sprintf("inonCeCx: %d, %s", inonCeCx, nonCeCx_vec[[inonCeCx]]) )
			print( "_difference_ in Costsave_nonCeCx; from Costsave_nonCeCx_array_byType_forSummary" )
				diff_Costsave_array_TEMP = as.matrix(Costsave_nonCeCx_array_byType_forSummary[[inonCeCx]][-1, iVdur, ]) - pracma::repmat(Costsave_nonCeCx_array_byType_forSummary[[inonCeCx]][1, iVdur, ], VC_boys_num-1, 1);
				print( t(apply(diff_Costsave_array_TEMP,1, simplify=TRUE, summary_new)) )
			print( "QALYgain_nonCeCx; QALYgain_nonCeCx_array_byType_forSummary" )
				diff_QALYgain_array_TEMP = as.matrix(QALYgain_nonCeCx_array_byType_forSummary[[inonCeCx]][-1, iVdur, ]) - pracma::repmat(QALYgain_nonCeCx_array_byType_forSummary[[inonCeCx]][1, iVdur, ], VC_boys_num-1, 1);
				print( t(apply(diff_QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )
			print( "LYgain_nonCeCx; LYgain_nonCeCxDeath_array_byType_forSummary" )
				diff_QALYgain_array_TEMP = as.matrix(LYgain_nonCeCxDeath_array_byType_forSummary[[inonCeCx]][-1, iVdur, ]) - pracma::repmat(LYgain_nonCeCxDeath_array_byType_forSummary[[inonCeCx]][1, iVdur, ], VC_boys_num-1, 1);
				print( t(apply(diff_QALYgain_array_TEMP,1, simplify=TRUE, summary_new)) )
		} # end-inonCeCx
...
#browser()
	...
	if (numDose_Vacc_new==2){ # non-negative QALY changes for 2dose GNV vs 2dose FOV
...
#	print( rowMeans(QALY_diff_vsNoVacc_positive_YN & !Cost_diff_vsNoVacc_positive_YN) )
	...
	if (numDose_Vacc_new==1 & !is.null(xlim_plot_numDose1)){ xlim_plot = xlim_plot_numDose1 }
	if (numDose_Vacc_new==1 & !is.null(ylim_plot_numDose1)){ ylim_plot = ylim_plot_numDose1 }
	...
	if (numDose_Vacc_new==1){
		...
		if (subplot_iVC_YN){ # scatter plot
			...
			for (iVC in 2:VC_boys_num){
				...
				mtext(text="1-dose GNV for girls and boys vs 2-dose FOV", side=3, line=1.5) # , adj=0.05 
				...
				yrange_plot = yrange_plot + diff(yrange_plot)/35*c(1,-1)
				if (TRUE){
					# Q1, and cost-effective
					prop_plot = c(mean(yplot>0 & xplot>0), mean((yplot/Cost_plotunit)/xplot<WTPthreshold & yplot>0 & xplot>0));
					leg_text = c(sprintf("Q1: %.1f%%",prop_plot[1]*100), sprintf("CE: %.1f%%", prop_plot[2]*100));
					leg_text = paste(leg_text, collapse="\n")
					text(x=xrange_plot[2], y=yrange_plot[2], labels=leg_text, adj=c(1,1), cex=1.45)

					# Q2
					prop_plot = mean(yplot>0 & xplot<0);
					leg_text = sprintf("Q2: %.1f%%",prop_plot[1]*100)
					# leg_text = paste(c(sprintf("Q2: %.1f%%",prop_plot[1]*100), sprintf("(Strongly dominated)")), collapse="\n");
					text(x=xrange_plot[1], y=yrange_plot[2], labels=leg_text, adj=c(0,1), cex=1.45)
	...
	if (numDose_Vacc_new==2){
		...
		if (subplot_iVC_YN){ # scatter plot
			...
			for (iVC in 2:VC_boys_num){
				...
				axis(1, col=NA, col.ticks='black', padj=-0.15, cex.axis=1.5)
				axis(2, at=par()$usr[3:4], label=FALSE, col.ticks=NA)
				axis(2, col=NA, col.ticks='black', las=1, hadj=0.9, cex.axis=1.5)
				mtext(side=1, xlab_plot, cex=1, line=2.5)
				mtext(side=2, ylab_plot, cex=1, line=3.5)

				mtext(text=sprintf("GNV %d%% uptake for boys", VC_boys_vec[iVC]), side=3, line=0.15) # , adj=0.05 
				mtext(text="2-dose GNV for girls and boys vs 2-dose FOV", side=3, line=1.5) # , adj=0.05 
				points(x=mean(xplot), y=mean(yplot), pch=pch_meanpt, col=col_meanpt, cex=cex_meanpt, lwd=lwd_meanpt)
				# confidence ellipse
				# car::dataEllipse(xplot, yplot, levels=0.9, lwd=1, col='blue', lty=2, plot.points=FALSE)
				...
				if (iVC==2){ # add text when plotting the first plot
					mtext(text=sprintf('iVdur=%d',iVdur), side=3, line=1.75, cex=0.7, adj=1, at=mtext_at)
					if (TRUE){ # if (calc_gwarts_YN){
						mtext(text=sprintf('gwarts=%s',ifelse(calc_gwarts_YN,"Y","N")), side=3, line=1, cex=0.7, adj=1, at=mtext_at)
					}
					if (run_SensAnaly_propMaleCx_YN){
						mtext(text="pMale_adj", side=3, line=0.25, adj=-0.2, cex=0.7)
					} 
				}
			...
			axis(1, col=NA, col.ticks='black', padj=-0.15, cex.axis=1)
			axis(2, at=par()$usr[3:4], label=FALSE, col.ticks=NA)
			axis(2, col=NA, col.ticks='black', las=1, hadj=0.9, cex.axis=1)
			mtext(side=1, xlab_plot, cex=1, line=2.5)
			mtext(side=2, ylab_plot, cex=1, line=3.75)
			...
			legend("bottomright", legend=paste0(VC_boys_vec[2:VC_boys_num], "%"), fill=adjustcolor(pch_col, alpha=0.75), title="Vaccine uptake\nin boys", bty="n", title.adj=0.5, title.cex=0.85)
	...
	TVC_inclAdminfee_Temp = TVCratio_Temp * Cost_Vacc_use_base / (numDose_Vacc_new/numDose_Vacc) 
	...
	# windows(width=num_subplot*5, height=5)
	if (numDose_Vacc_new==2){
		itype_CEAC_vec = 3
		scale_oneplotCEAC = 2;
		xylab_cex = 1.35
	...
	for (itype_CEAC in itype_CEAC_vec){
		plot(NA, ylim=c(0,1), xlim=c(0, WTP_vec_max), xlab="",ylab="", axes=FALSE, xaxs="i", yaxs="i" )
		segments(x0=WTPthreshold/ifelse(plot_Cost_USD_YN,7.8,1), y0=0,y1=1, col="grey80", lwd=1.5, lty=2)
		xtick_at = with(par(), seq(xaxp[1], xaxp[2], length.out=xaxp[3]+1))
		xtick_at_lab = format(xtick_at, big.mark=",", scientific=FALSE, trim=TRUE)
		axis(1, at=xtick_at, labels=xtick_at_lab, cex.axis=1.35)
		mtext(side=1, text=xlab_plot, line=2.5, cex=xylab_cex)
		ytick_at = seq(0,1, by=0.2)
		axis(2, at=ytick_at, las=1, cex.axis=1.35)
		if (itype_CEAC==3 | itype_CEAC==2){
		...
		for (iVC in 2:VC_boys_num){
			ceac_cost = Cost_diff_vsNoVacc_plot[iVC,];
			ceac_health = QALY_diff_vsNoVacc[iVC,];
			cost_unit_multiplier = ifelse(plot_Cost_USD_YN,7.8,1)
			
			ceac_nmb = do.call(cbind, sapply(WTP_vec, simplify=FALSE, function(xWTP) (xWTP*Cost_plotunit*cost_unit_multiplier)*ceac_health - ceac_cost));
			if (itype_CEAC==3){
				# the usual CEAC
				ceac_probCE = colMeans(1*(ceac_nmb>0))
			} else if (itype_CEAC==1){
				# no points in Q3 (SW) would be considered adoptable
				ceac_probCE = apply(ceac_nmb, 2, function(nmb) mean(nmb>0 & ceac_health>0))
			} else if (itype_CEAC==2){
				# points in Q3 (SW) would be considered adoptable if diff_cost/diff_QALY > WTA (WTA = WTA_WTP_ratio x WTP)
				WTA_factor = ifelse(ceac_health>=0, 1, WTA_WTP_ratio)
				ceac_nmb = do.call(cbind, sapply(WTP_vec, simplify=FALSE, function(xWTP) 
				(xWTP*WTA_factor)*Cost_plotunit*cost_unit_multiplier*ceac_health - ceac_cost))
				ceac_probCE = colMeans(1*(ceac_nmb>0))
			}
			lines(x=WTP_vec, y=ceac_probCE, lwd=2, col=col_ceac_vec[iVC], xpd=TRUE)
		} # for-iVC
		...
		if (numDose_Vacc_new==2){
			legend(ifelse(numDose_Vacc_new==2, "topleft","bottomright"), legend=sprintf("%d%%", VC_boys_vec[2:VC_boys_num]), title="Uptake among boys", inset=c(ifelse(numDose_Vacc_new==2, 0.025,0),0.025), col=col_ceac_vec[2:VC_boys_num], lwd=2, cex=1, xpd=TRUE)
		}
		...
		if (numDose_Vacc_new==1){
			...
			} else if (itype_CEAC==3){
				mtext(text="WTA = WTP", side=3, line=0.5)
			}
			
			# legend
			if (itype_CEAC==3){
				legend(ifelse(numDose_Vacc_new==2, "topleft","bottomright"), legend=sprintf("%d%%", VC_boys_vec[2:VC_boys_num]), title="Uptake among boys", inset=c(ifelse(numDose_Vacc_new==2, 0.025,0),0.025), col=col_ceac_vec[2:VC_boys_num], lwd=2, cex=1.35, xpd=TRUE)
			}
		}
	} # for - itype_CEAC
...
Cost_1dGNV_list = list(
	Cost_vaccGirls_array = Cost_vaccGirls_array_1dGNV,
	Cost_vaccBoys_array = Cost_vaccBoys_array_1dGNV,

	Cost_array = Cost_array_1dGNV,
	
	Costsave_nonCeCx_array = Costsave_nonCeCx_array_1dGNV ,
	Costsave_nonCeCx_array_byType = Costsave_nonCeCx_array_byType_1dGNV,
	Costsave_nonCeCx_array_byType_noMaleCxFOV = Costsave_nonCeCx_array_byType_noMaleCxFOV_1dGNV,
	
	Costsave_gwarts_array_sumsex_1dGNV = Cost_overall_array_1dGNV,
	
	Cost_overall_array = Cost_overall_array_1dGNV,
	Cost_overall_array_gwarts = Cost_overall_array_gwarts_1dGNV,

	Cost_overall_array_woVacc = Cost_overall_array_woVacc_1dGNV, 
	Cost_overall_array_onlyVacc_girlsboys_noadjust = Cost_overall_array_onlyVacc_girlsboys_noadjust_1dGNV
)
QALY_1dGNV_list = list(

	QALY_array = QALY_array_1dGNV,
	
	QALYgain_nonCeCx_array = QALYgain_nonCeCx_array_1dGNV,
	LYgain_nonCeCxDeath_array = LYgain_nonCeCxDeath_array,

	QALYgain_nonCeCx_array_byType = QALYgain_nonCeCx_array_byType_1dGNV,
	QALYgain_nonCeCx_array_byType_noMaleCxFOV = QALYgain_nonCeCx_array_byType_noMaleCxFOV_1dGNV,
	LYgain_nonCeCxDeath_array_byType = LYgain_nonCeCxDeath_array_byType_1dGNV,
	LYgain_nonCeCxDeath_array_byType_noMaleCxFOV = LYgain_nonCeCxDeath_array_byType_noMaleCxFOV_1dGNV,

	QALYgain_gwarts_array_sumsex = QALYgain_gwarts_array_sumsex_1dGNV,

	QALY_overall_array = QALY_overall_array_1dGNV,
	QALY_overall_array_gwarts = QALY_overall_array_gwarts_1dGNV
)
