...
RData_vec = sapply(RData_input_list, function(x) do.call(sprintf, c(fmt="estoutput_%s_sensAnaly_fit_CxInc_%s_%s.RData", as.list(x)))) # <-- include file suffix
...
for (ilist in iparamest_list_vec){
	...
	if (adjrelative_param_YN){ # be careful
		...
		# duration MCMCout_estparam_temp[,(1-1)*nparam_iset+1]*MCMCout_estparam_temp[,(1-1)*nparam_iset+2]
		meandur_MCMCout_estparam_temp = cbind(
			MCMCout_estparam_temp[,1]*MCMCout_estparam_temp[,2]
			,MCMCout_estparam_temp[,6]*MCMCout_estparam_temp[,7]
			,MCMCout_estparam_temp[,11]*MCMCout_estparam_temp[,12] 
			
			,MCMCout_estparam_temp[,3]/MCMCout_estparam_temp[,8]
			,MCMCout_estparam_temp[,4]/MCMCout_estparam_temp[,9]
			,MCMCout_estparam_temp[,5]/MCMCout_estparam_temp[,10]
			
			,MCMCout_estparam_temp[,3]/MCMCout_estparam_temp[,13]
			,MCMCout_estparam_temp[,4]/MCMCout_estparam_temp[,14]
			,MCMCout_estparam_temp[,5]/MCMCout_estparam_temp[,15]
			)
		}		
...
for (ilist in iparamest_list_vec){
	...
	if (mcmc_thin_YN){ # thinning of MCMC outputs
		if (nrow(CxInc_estparam_temp) == nrow(MCMCout_estparam_list[[ilist]])){ # if not equal, then MCMCout_CxInc_list may be re-calculated and no need to thin
